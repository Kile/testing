...
export async function unknownCommand(message: discord.Message): Promise<void> {
  ...
  await setTimeout(async () => await msg?.delete(), 5000);
}
...
export async function relax(message: discord.Message): Promise<discord.Message> {
  const u = await new User(+message.author.id).getData();
...
export function info(item: number): discord.Embed | string {
  let data = (ITEMS as any)[item.toString()];
